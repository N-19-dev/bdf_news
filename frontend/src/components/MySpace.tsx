  ...
  if (!user) {
    return (
      <div className="text-center py-12">
        <p className="text-neutral-500">Connecte-toi pour acc&eacute;der &agrave; ton espace personnel.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex gap-2 border-b border-neutral-200">
        ...
        >
          Sauvegard&eacute;s ({savedArticles.length})
        </button>
      </div>
        ...
        <div className="space-y-4">
          ...
          {isLoading ? (
            ...
            <p className="text-center text-neutral-500 py-8">
              Tape un mot-cl&eacute; pour rechercher dans {allArticles.length} articles
            </p>
          ) : searchResults.length === 0 ? (
            <p className="text-center text-neutral-500 py-8">Aucun r&eacute;sultat</p>
          ) : (
      ...
      {activeTab === "saved" && (
        <div className="space-y-2">
          {savedArticles.length === 0 ? (
            <p className="text-center text-neutral-500 py-8">
              Aucun article sauvegard&eacute;
            </p>
          ) : (
...
function ArticleRow({
  ...
  article: { url: string; title: string; source_name: string; published_ts: number };
  isSaved: boolean;
  onToggle: () => void;
}) {
  return (
    <div className="flex items-center gap-3 p-3 bg-white rounded-lg border border-neutral-200 hover:border-neutral-300 transition">
      ...
      >
        ...
        <p className="text-xs text-neutral-500">
          {article.source_name} &middot; {formatDate(article.published_ts)}
        </p>
      </a>
      <button
        onClick={(e) => {
